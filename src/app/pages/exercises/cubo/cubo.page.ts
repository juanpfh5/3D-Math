...
export class CuboPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de un cubo cuya longitud del cuadrado es <strong>s = ${num1}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 4 * num1;
        ruta = 'assets/images/cubo/fig_1.jpg';
        break;
      case 1: // Área de la base
        respuesta = Math.pow(num1, 2);
        ruta = 'assets/images/cubo/fig_2.jpg';
        break;
      case 2: // Área lateral
        respuesta = 4 * Math.pow(num1, 2);
        ruta = 'assets/images/cubo/fig_3.jpg';
        break;
      case 3: // Área total
        respuesta = 2 * Math.pow(num1, 2);
        ruta = 'assets/images/cubo/fig_4.jpg';
        break;
      case 4: // Volumen
        respuesta = Math.pow(num1, 3);
        ruta = 'assets/images/cubo/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/cubo/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      ...
      <p><strong>Área lateral</strong>: <br> <span>$$A_{lateral} = 4 s^2$$</span></p>
      <p><strong>Área total</strong>: <br> <span>$$A_{total} = 2 s^2$$</span></p>
      <p><strong>Volumen</strong>: <br> <span>$$V = s^3$$</span></p>
      Donde <strong>s</strong> es la longitud de un lado del cuadrado.
    `;
