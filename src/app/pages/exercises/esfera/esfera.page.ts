...
export class EsferaPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una esfera cuyo radio es <strong>r = ${num1}</strong> y cuya altura es <strong>h = ${num3}</strong>. Toma en consideranción que el valor de π será <strong>π = ${num2}</strong>.`;
  ...
  async presentHelpPopover(ev: Event) {
    ...
    const formulas = `
      <p><strong>Perímetro (Circunferencia)</strong>: <br> <span>$$ P = 2 \\pi r $$</span></p>
      <p><strong>Área de la base</strong>: <br> <span>$$ A = 4 \\pi r^2 $$</span></p>
      <p><strong>Volumen</strong>: <br> <span>$$ V = \\frac{4}{3} \\pi r^3 $$</span></p>
      <p>Donde <strong>r</strong> es el radio de la esfera y <strong>π</strong> (pi) es una constante aproximadamente igual a 3.1416.</p>
    `;
