...
export class PiramideCuadrangularPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una pirámide cuadrangular cuya longitud de cada lado del cuadrado es <strong> a = ${num1}</strong>, cuyo valor de la apotema es <strong>ap = ${num2}</strong> y cuya altura es igual a <strong>h = ${num3}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 4 * num1;
        ruta = 'assets/images/piramide_cuadrangular/fig_1.jpg';
        break;
      case 1: // Área de la base
        respuesta = Math.pow(num1, 2);
        ruta = 'assets/images/piramide_cuadrangular/fig_2.jpg';
        break;
      case 2: // Área lateral
        respuesta = 2 * num1 * num2;
        ruta = 'assets/images/piramide_cuadrangular/fig_3.jpg';
        break;
      case 3: // Área total
        respuesta = Math.pow(num1, 2) + 2 * num1 * num2;
        ruta = 'assets/images/piramide_cuadrangular/fig_4.jpg';
        break;
      case 4: // Volumen
        respuesta = (1/3) * Math.pow(num1, 2) * num3;
        ruta = 'assets/images/piramide_cuadrangular/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/piramide_cuadrangular/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      <p><strong>Perímetro de la base</strong>: <br> <span>$$P = 4 a$$</span></p>
      <p><strong>Área de la base</strong>: <br> <span>$$A_{base} = a^2$$</span></p>
      <p><strong>Área lateral</strong>: <br> <span>$$A_{lateral} = 2 a * ap$$</span></p>
      <p><strong>Área total</strong>: <br> <span>$$A_{total} = A_{base} + A_{lateral}$$</span></p>
      <p><strong>Volumen</strong>: <br> <span>$$V = \\frac{1}{3} a^2 h$$</span></p>
      Donde <strong>a</strong> es la longitud de un lado del cuadrado, <strong>ap</strong> es la apotema (la altura de cada cara triangular desde el centro de la base hasta el vértice de la pirámide) y <strong>h</strong> es la altura desde el centro de la base cuadrada hasta el vértice de la pirámide.
    `;
