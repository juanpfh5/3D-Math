...
export class PiramideEneagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una pirámide eneagonal cuya longitud de cada lado del eneágono es <strong> a = ${num1}</strong>, cuyo valor de la apotema es <strong>ap = ${num2}</strong> y cuya altura es igual a <strong>h = ${num3}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 9 * num1;
        ruta = 'assets/images/piramide_eneagonal/figure_static.jpg';
        break;
      case 1: // Área de la base
        pregunta = pregunta + ` (Considere que <strong>pi = ${num4}</strong>).`;
        respuesta = (9 / 4) * Math.pow(num1, 2) * (1 / Math.tan(num4 / 9));
        ruta = 'assets/images/piramide_eneagonal/figure_static.jpg';
        break;
      case 2: // Área lateral
        respuesta = (9/2) * num1 * num2;
        ruta = 'assets/images/piramide_eneagonal/figure_static.jpg';
        break;
      case 3: // Área total
        pregunta = pregunta + ` (Considere que <strong>pi = ${num4}</strong>).`;
        respuesta = (9 / 4) * Math.pow(num1, 2) * (1 / Math.tan(num4 / 9)) + (9/2) * num1 * num2;
        ruta = 'assets/images/piramide_eneagonal/figure_static.jpg';
        break;
      case 4: // Volumen
        pregunta = pregunta + ` (Considere que <strong>pi = ${num4}</strong>).`;
        respuesta = (1 / 3) * ((9 / 4) * Math.pow(num1, 2) * (1 / Math.tan(num4 / 9))) * num3;
        ruta = 'assets/images/piramide_eneagonal/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/piramide_eneagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      <p><strong>Perímetro de la base</strong>: <br> <span>$$P = 9 a$$</span></p>
      <p><strong>Área de la base</strong>: <br> <span>$$A_{base} = \\frac{9}{4} a^2 cot(\\frac{\\pi}{9})$$</span></p>
      <p><strong>Área lateral</strong>: <br> <span>$$A_{lateral} = \\frac{9}{2} a * ap$$</span></p>
      ...
      Donde <strong>a</strong> es la longitud de un lado del eneágono, <strong>ap</strong> es la apotema de una cara triangular lateral (la altura de cada triángulo lateral desde la base hasta el vértice de la pirámide) y <strong>h</strong> es la altura de la pirámide.
    `;
