...
export class PiramideHexagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una pirámide hexagonal cuya longitud de cada lado del hexágono es <strong> a = ${num1}</strong>, cuyo valor de la apotema es <strong>ap = ${num2}</strong> y cuya altura es igual a <strong>h = ${num3}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 6 * num1;
        ruta = 'assets/images/piramide_hexagonal/fig_1.jpg';
        break;
      case 1: // Área de la base
        respuesta = (3 * Math.sqrt(3) / 2) * Math.pow(num1, 2);
        ruta = 'assets/images/piramide_hexagonal/fig_2.jpg';
        break;
      case 2: // Área lateral
        respuesta = 3 * num1 * num2;
        ruta = 'assets/images/piramide_hexagonal/fig_3.jpg';
        break;
      case 3: // Área total
        respuesta = (3 * Math.sqrt(3) / 2) * Math.pow(num1, 2) + 3 * num1 * num2;
        ruta = 'assets/images/piramide_hexagonal/fig_4.jpg';
        break;
      case 4: // Volumen
        respuesta = (1/3) * (3 * Math.sqrt(3) / 2) * Math.pow(num1, 2) * num3;
        ruta = 'assets/images/piramide_hexagonal/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/piramide_hexagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      ...
      <p><strong>Área lateral</strong>: <br> <span>$$A_{lateral} = 3 a * ap$$</span></p>
      ...
      Donde <strong>a</strong> es la longitud de un lado del hexágono, <strong>ap</strong> es la apotema (la altura de cada triángulo lateral desde la base hasta el vértice de la pirámide) y <strong>h</strong> es la altura desde el centro de la base hexagonal hasta el vértice de la pirámide.
    `;
