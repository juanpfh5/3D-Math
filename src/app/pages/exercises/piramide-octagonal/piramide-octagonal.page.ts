...
export class PiramideOctagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una pirámide octagonal cuya longitud de cada lado del octágono es <strong> a = ${num1}</strong>, cuyo valor de la apotema es <strong>ap = ${num2}</strong> y cuya altura es igual a <strong>h = ${num3}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 8 * num1;
        ruta = 'assets/images/piramide_octagonal/fig_1.jpg';
        break;
      case 1: // Área de la base
        respuesta = 2 * (1 + Math.sqrt(2)) * Math.pow(num1, 2);
        ruta = 'assets/images/piramide_octagonal/figure_static.jpg';
        break;
      case 2: // Área lateral
        respuesta = 4 * num1 * num2;
        ruta = 'assets/images/piramide_octagonal/fig_1.jpg';
        break;
      case 3: // Área total
        respuesta = 2 * (1 + Math.sqrt(2)) * Math.pow(num1, 2) + (4 * num1 * num2);
        ruta = 'assets/images/piramide_octagonal/figure_static.jpg';
        break;
      case 4: // Volumen
        respuesta = (1 / 3) * (2 * (1 + Math.sqrt(2)) * Math.pow(num1, 2)) * num3;
        ruta = 'assets/images/piramide_octagonal/fig_1.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/piramide_octagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      <p><strong>Perímetro de la base</strong>: <br> <span>$$P = 8 a$$</span></p>
      <p><strong>Área de la base</strong>: <br> <span>$$A_{base} = 2 (1 + \\sqrt{2}) a^2$$</span></p>
      <p><strong>Área lateral</strong>: <br> <span>$$A_{lateral} = 4 a * ap$$</span></p>
      ...
      Donde <strong>a</strong> es la longitud de un lado del octágono, <strong>ap</strong> es la apotema (la altura de cada triángulo lateral desde la base hasta el vértice de la pirámide) y <strong>h</strong> es la altura desde el centro de la base octagonal hasta el vértice de la pirámide.
    `;
