...
export class PrismaEneagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de una pirámide eneagonal cuya longitud de cada lado del eneágono es <strong> a = ${num1}</strong> y cuya altura es igual a <strong>h = ${num2}</strong>. (Considere que <strong>pi = ${num3}</strong>).`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 9 * num1;
        ruta = 'assets/images/prisma_eneagonal/figure_static.jpg';
        break;
      case 1: // Área de la base
        respuesta = (9/4) * Math.pow(num1, 2) * (1/Math.tan(num3/9));
        ruta = 'assets/images/prisma_eneagonal/figure_static.jpg';
        break;
      case 2: // Área lateral
        respuesta = 9 * num1 * num2;
        ruta = 'assets/images/prisma_eneagonal/figure_static.jpg';
        break;
      case 3: // Área total
        respuesta = 2 * ((9/4) * Math.pow(num1, 2) * (1/Math.tan(num3/9))) + (9 * num1 * num2);
        ruta = 'assets/images/prisma_eneagonal/figure_static.jpg';
        break;
      case 4: // Volumen
        respuesta = (9/4) * Math.pow(num1, 2) * (1/Math.tan(num3/9)) * num2;
        ruta = 'assets/images/prisma_eneagonal/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/prisma_eneagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      ...
      Donde <strong>a</strong> es la longitud de un lado del eneágono, <strong>π</strong> (pi) es una constante aproximadamente igual a 3.1416 y <strong>h</strong> es la altura del prisma.
    `;
