...
export class PrismaOctagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de un prisma octagonal cuya longitud de cada lado del octágono es <strong> a = ${num1}</strong> y cuya altura es igual a <strong>h = ${num2}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 8 * num1;
        ruta = 'assets/images/prisma_octagonal/figure_static.jpg';
        break;
      case 1: // Área de la base
        ...
        break;
      case 2: // Área lateral
        respuesta = 8 * num1 * num2;
        ruta = 'assets/images/prisma_octagonal/figure_static.jpg';
        break;
      case 3: // Área total
        respuesta = 2 * 2 * (1 + Math.sqrt(2)) * Math.pow(num1, 2) + 8 * num1 * num2;
        ruta = 'assets/images/prisma_octagonal/figure_static.jpg';
        break;
      case 4: // Volumen
        respuesta = 2 * (1 + Math.sqrt(2)) * Math.pow(num1, 2) * num2;
        ruta = 'assets/images/prisma_octagonal/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/prisma_octagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      ...
      Donde <strong>a</strong> es la longitud de un lado del octágono y <strong>h</strong> es la altura del prisma.
    `;
