...
export class PrismaPentagonalPage implements OnInit {
  ...
  generateRandomOperation(option: number): { pregunta: string, respuesta: string, ruta: string } {
    ...
    let pregunta = `Calcular ${operation} de un prisma pentagonal cuya longitud de cada lado del pentágono es <strong> a = ${num1}</strong> y cuya altura es igual a <strong>h = ${num2}</strong>.`;
    ...
    switch (option) {
      case 0: // Perímetro de la base
        respuesta = 5 * num1;
        ruta = 'assets/images/prisma_pentagonal/fig_1.jpg';
        break;
      case 1: // Área de la base
        respuesta = (1/4) * Math.sqrt(5 * (5 + 2 * Math.sqrt(5))) * Math.pow(num1, 2);
        ruta = 'assets/images/prisma_pentagonal/fig_2.jpg';
        break;
      case 2: // Área lateral
        respuesta = 5 * num1 * num2;
        ruta = 'assets/images/prisma_pentagonal/fig_3.jpg';
        break;
      case 3: // Área total
        respuesta = (1/4) * Math.sqrt(5 * (5 + 2 * Math.sqrt(5))) * Math.pow(num1, 2) + 5 * num1 * num2;
        ruta = 'assets/images/prisma_pentagonal/fig_4.jpg';
        break;
      case 4: // Volumen
        respuesta = (1/4) * Math.sqrt(5 * (5 + 2 * Math.sqrt(5))) * Math.pow(num1, 2) * num2;
        ruta = 'assets/images/prisma_pentagonal/figure_static.jpg';
        break;
      default:
        respuesta = 0;
        ruta = 'assets/images/prisma_pentagonal/figure.gif';
        break;
    }
  ...
  async presentHelpPopover(ev: Event) {
    const formulas = `
      <p><strong>Perímetro de la base</strong>: <br> <span>$$P = 5 s$$</span></p>
      <p><strong>Área de la base</strong>: <br> <span>$$A_{base} = \\frac{1}{4} \\sqrt{5 (5 + 2 \\sqrt{5})} s^2$$</span></p>
      ...
      Donde <strong>s</strong> es la longitud de un lado del pentágono y <strong>h</strong> es la altura del prisma.
    `;
